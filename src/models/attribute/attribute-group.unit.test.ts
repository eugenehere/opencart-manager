...
const model = new AttributeGroupModel(fakeDatabase);

let id: number;
const table: string = "attribute_group";
const data = { sortOrder: 0 };
const updated = { ...data, sortOrder: 1 };
const criteria = { attributeGroupId: -1 };
const initialValues = initialAttributeGroup;

test("insert", async () => {
  const result = await model.insert(data);
  expect(result).toBeResponseHeaders();
  criteria.attributeGroupId = id = result.insertId;
